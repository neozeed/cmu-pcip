...
/*
11-21-85 Added check for stack overflow on initial task in tk_init
	if compiled under MSC.
					<Drew D. Perkins>
3-21-86 Changed initial task stack allocation to allocate the entire
	DOS stack.  It will now allocate a larger stack than was asked
	for, but will print an error if it can't allocate one atleast
	as much as was asked for.
					<Drew D. Perkins>
 */
...
static task *died;
#ifdef MSC				/* DDP */
	extern int end; 		/* DDP - End of data in MSC */
#endif					/* DDP */
...
#else
	tk->tk_guard = (unsigned *)(&end); /* Start of the stack */

	/* fill the system stack with the guardword, and check actual size */
	tk->tk_size = (int)_stk_sys_fill();
	if (tk->tk_size < stksiz) {
		printf("tk_init: Not enough memory for initial stack.\n");
		printf("%d bytes requested but only %d bytes available\n",
			stksiz, tk->tk_size);
		exit(1);
	}
#ifdef	DEBUG
	if(TDEBUG)
		printf("tk_init: Initial stack size = %d\n", tk->tk_size);
#endif
#endif				/* DDP - End */
