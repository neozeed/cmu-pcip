...
struct ip {
	unsigned char ip_ihl : 4; /* Internet header length in 32 bit words */
	unsigned char ip_ver : 4; /* Header version */
...
struct ip_iob {
	...
	int	(*du_handle)();	/* upcalling destination unreachable handler*/
	};
...
#ifdef LPKT			/* DDP/LKR */
#define INETLEN 1280		/* DDP/LKR high performance packet size    */
#else				/* DDP/LKR */
