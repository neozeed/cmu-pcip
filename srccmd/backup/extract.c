...
extract_archive()
{
	...
	again_file:
		fd = open(head->header.name,
			f_keep?
#ifdef	MSDOS
			O_NDELAY|O_WRONLY|O_APPEND|O_CREAT|O_EXCL|O_BINARY:
			O_NDELAY|O_WRONLY|O_APPEND|O_CREAT|O_TRUNC|O_BINARY,
#else
	...
	case LF_LINK:
#ifdef	MSDOS
		fprintf(stderr, "Link %s to %s ignored\n",
			head->header.linkname, head->header.name);
#else
	again_link:
	...
	make_node:
#ifdef	MSDOS
		fprintf(stderr, "Node %s type %x ignored\n",
			head->header.name, hstat->st_mode);
#else
		check = mknod(head->header.name, (int) hstat->st_mode,
#ifdef	OS
			major(hstat->st_dev), minor(hstat->st_dev));
#else
			(int) hstat->st_dev);
#endif
		if (check != 0) {
