...
IcmpInit() {
	...
	icmp = in_open(ICMP, icmprcv, NULL);

#ifdef	DEBUG
	if(icmp == 0 && (NDEBUG & IPTRACE))
		printf("ICMP: Can't open ip conn\n");
	else if(NDEBUG & IPTRACE)
...
static icmprcv(p, len, host)
...
#ifdef	DEBUG
		if(NDEBUG & (IPTRACE|PROTERR|NETERR)) {
			printf("ICMP: got dest unreachable type ");
			printf("%u on %a from %a\n", pdp->dcode,
				pdp->dip.ip_dest, host);
			}
#endif
		indudemux(&pdp->dip);	/* DDP - upcall handler */
		in_free(p);
		break;
	case SOURCEQ:
#ifdef	DEBUG
...
#ifdef	DEBUG
		if(NDEBUG & IPTRACE)
			printf("ICMP: rcvd redirect for %a to %a\n",
			       rd->rdip.ip_dest, rd->rdgw);
#endif
	...
	case ECHOREP:
		...
		if(e->pseq != pingseq-1) {
#ifdef DEBUG
			if(NDEBUG & (NETERR|PROTERR|IPTRACE))
...
IcEchoRequest(host, length)
	...
	if((NDEBUG & IPTRACE) && (NDEBUG & DUMP))
		in_dump(p);
#endif

	while(pingstate == PGWAITING)
		tk_block();
