...
p_hardware() {
	...
	printf("Base memory is %x,%x\n\033K", custom.c_basemem);
...
set_basemem() {
	...
	printf("\r\033K");
	get_line("Enter the base memory address for the net interface in hex ",
							buf, 80);
	sscanf(buf, "%lx", &custom.c_basemem);
	return 0;
	}
...
struct menu_ent menu_hardware[] = {
	...
	'm'," -m\tSet net interface memory base address", set_basemem, 0,
