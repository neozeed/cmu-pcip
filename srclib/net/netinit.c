...
/* DDP end */
	...
	if(nets[0].n_init == NULL) {
		printf("BUGHALT: no interface initializer\n");
		exit(1);
		}
	...
	if((custom.c_me & AMASK) == AADDR) {
		nets[0].n_netbr = (custom.c_me & 0xff) | 0xffffff00;
		nets[0].n_netbr42 = custom.c_me & 0xff;
	}
	else if((custom.c_me & BMASK) == BADDR) {
		nets[0].n_netbr = (custom.c_me & 0xffff) | 0xffff0000;
		nets[0].n_netbr42 = custom.c_me & 0xffff;
	}
	else if((custom.c_me & CMASK) == CADDR) {
		nets[0].n_netbr = (custom.c_me & 0xffffff) | 0xff000000;
		nets[0].n_netbr42 = custom.c_me & 0xffffff;
	}
	nets[0].n_subnetbr = custom.c_me | ~custom.c_net_mask;

	if(((nets[0].ip_addr & ~(nets[0].n_custom->c_net_mask)) == 0) &&
