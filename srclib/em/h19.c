...
/*  h19 terminal emulator
	...
	I/O library.
						<John Romkey>
    10/17/85 - Mask off the high bit so we don't get graphics characters!
    						<Drew D. Perkins>
    11/19/86 - Major rewrite of sh.asm to support "snowless" color card
	operation.
	Pass extra position argument in [n]write_char().
	move_lines() now inherently does a clear_lines().
    						<Drew D. Perkins>
*/
...
char	data;
{
  ...
  int	line_buf[80];	/* buffer for lines */
  	...
  	data &= 0x7f;	/* DDP - Mask off high bit (No graphics!) */
	switch (data) {
	case ESC:
	...
	default:	/* send characters to screen */
		...
		write_char(data | (attrib << 8), pos, (x_pos | (y_pos << 8)));
		x_pos++;
		pos++;
...
/* DDP - use BIOS function instead.
	read_line(line_buf,y_pos);
	for (count = x_pos; count <= 79; count++)
		line_buf[count] = SPACE | (NORM_VIDEO << 8);
	write_line(line_buf,y_pos);
*/
	/* DDP - Clear from x to 79 on line y */
	clear_chars(y_pos, x_pos, 79, NORM_VIDEO);
...
/* DDP - move_lines now does an inherent clear_lines
	clear_lines(y_pos,1);
 */
  	break;
  case 'M':		/* delete line */
...
/* DDP - use BIOS function instead.
	...
	write_line(line_buf,y_pos);
*/
	/* DDP - Clear from 0 to x-1 on line y */
	clear_chars(y_pos, 0, x_pos-1, NORM_VIDEO);
  	break;
 ...
 */
		...
		break;
	case 'M':	/* Delete Lines */
		move_lines(y_pos + arg, y_pos, 25 - arg - y_pos);
/* DDP - move_lines now does an inherent clear_lines
