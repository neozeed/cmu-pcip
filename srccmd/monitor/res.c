 ...
 *	res_name	Resolve a name as specified above
 *	xgethmch	Parse a hex machine address specification
 *	xgetomch	Parse an octal machine address specification
 *	xgetdmch	Parse a decimal machine address specification
 */
...
extern	long	xgetomch();
extern	long	xgetdmch();
extern	long	xgethmch();
...
long res_name (name)
	...
	if((name[0] >= '0') && (name[0] <= '9')) 
		if (name[1] == '.' ||
		    name[2] == '.' ||
		    name[3] == '.')
		    return xgetdmch(name);
		    else return xgetomch(name);
		
	else if(name[0] == '#')
		return xgethmch(name);
