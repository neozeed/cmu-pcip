...
/* 7/9/84 - added crock to accept 4.2 broadcast packets. Probably should
	have a per net broadcast address in the net structure.
					<John Romkey>
  8/7/86 - fixed crock!  Now have a per net broadcast address, a per net
	subnet broadcast address, and a per net 4.2bsd style broadcast
	address in net structure!  Also fixed crock not to send icmp dest
	unreachables on broadcasts.
					<Drew D. Perkins>
...
indemux(p, len, nt)
	...
	if((pip->ip_dest != nt->ip_addr) && /* Quick check on our own addr */
	   (pip->ip_dest != 0xffffffff) && /* Physical cable broadcast addr*/
